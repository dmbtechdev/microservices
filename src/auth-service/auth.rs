...
impl Auth for AuthService {
    async fn sign_in(
        ...
        println!("Got a sign_in request: username {:?}; password '*****'", req.username);

        let result: Option<String> = //todo!(); // Get user's uuid from `users_service`. Panic if the lock is poisoned.
            self.users_service.lock().unwrap().get_user_uuid(req.username, req.password);
        ...
        match result {
            None => {
                reply = //todo!(); // Create a `SignInResponse` with `status_code` set to `Success`
                    SignInResponse {
    ...
    async fn sign_up(
        ...
        let req = request.into_inner();
        println!("Got a sign_up request: username {:?}; password '*****'", req.username);
